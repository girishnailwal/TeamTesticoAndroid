package com.teamtasticoapp.database;
...
/**
 * Created by mobulous55 on 15/1/18.
 */
...
public class SharedPreferenceWriter {
    ...
    public static SharedPreferenceWriter getInstance(Context c) {
        ...
        if (null == sharePref) {
            ...
            mPrefs = context.getSharedPreferences("unisocial", Context.MODE_PRIVATE);
    ...
    public void writeStringValueList(String key, Set<String> list) {
        if (prefEditor != null) {
            prefEditor.putStringSet(key, list);
            prefEditor.commit();
        }
